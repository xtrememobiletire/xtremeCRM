...
## Phase 6: Zadarma Telephony & Cloud PBX Integration
- **Objective:** Zero-seat-cost telephony integration using Zadarma Cloud PBX, multi-country DIDs, and instant screen-pop without raw WebRTC engineering.
- **Deliverables:**
  - Zadarma Webhook handler (`POST /api/telephony/zadarma-webhook`): Receives `NOTIFY_START` / `NOTIFY_INTERNAL` events and emits Socket.io screen-pop event to the agent's screen.
  - Extension mapping: Links Zadarma extension (e.g. `101`) to agent `User.telephonyExtension`.
  - Embedded Zadarma Webphone drawer in React + Click-to-dial callback trigger on customer/job records.
  - Call recording synchronization on `NOTIFY_END`: Saves MP3 recording link to `Job.callRecordingUrl`.
